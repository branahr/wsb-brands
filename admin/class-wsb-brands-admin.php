	...
	**/
	...
	function brands_edit_fields( $term ) {
        ...
        <tr class="form-field term-group-wrap">
     		...
     		<td>
       			<input type="hidden" id="brandLogo" name="brandLogo" value="<?php echo $logo; ?>">
       			<div id="brand-logo-wrapper">
         		<?php if ( $logo ) { ?>
           		<?php echo wp_get_attachment_image ( $logo, 'medium', true, array( 'style' => 'max-height:100px !important; width: auto! important;') ); ?>
         		<?php } ?>
       			</div>
       			<p>
         		<input type="button" class="button button-secondary wsb_brands_media_button" id="wsb_brands_media_button" name="wsb_brands_media_button" value="<?php _e( 'Add Logo', 'wsb-brands' ); ?>" />
        ...
        <tr class="form-field">
			...
			<th><label for="brandUrl"><?php _e( 'Website', 'wsb-brands' ); ?></label></th>
				...
				<td>	 
					<input type="text" name="brandUrl" id="brandUrl" value="<?php echo esc_attr( $url ); ?>" placeholder="<?php echo __( 'Enter full url (Example: http://www.google.com)', 'wsb-brands' ); ?>">
				</td>
		</tr>
		<?php
	}
